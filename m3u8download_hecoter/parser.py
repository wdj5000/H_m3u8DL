...
class Parser:
    def __init__(
            self,m3u8url,title='',method=None,key=None,iv=None,work_dir='./Downloads',headers={
    ...
    def run(self):
        ...
        if 'key' in segments[0]:
            self.method,segments = decrypt.Decrypt(m3u8obj,self.temp_dir,method=self.method,key=self.key,iv=self.iv).run()
