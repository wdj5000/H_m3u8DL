...
class Parser:
    ...
    def guess_title(self,m3u8url):
        ...
        def youku():
            ...
            response = requests.get(parse_url,verify=False,proxies=self.proxies).json()
            title = response['data']['video']['title']
            return title
        if 'youku.com' in m3u8url:
            title = youku()
        else:
    ...
    def run(self):
        ...
        for i, segment in enumerate(segments):
            ...
            # bug
            # if 'init_section' in segment:
            #     segment['uri'] = segment['init_section']['uri']
