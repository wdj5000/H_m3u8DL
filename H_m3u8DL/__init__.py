...
def m3u8download(m3u8url, title='',base_uri_parse='',threads=16, key=None, iv=None, method=None, work_dir=None, headers=None,enable_del=True,merge_mode=3,proxy=None):
    ...
    if '.mp4' in m3u8url and 'm3u8' not in m3u8url:
        ...
        if type(m3u8url) == list:
            infos = parser.Parser('master').resume(m3u8url)
            for info in infos:
                m3u8download(m3u8url=info['m3u8url'], title='' if 'title' not in info else info['title'],base_uri_parse=None if 'base_uri_parse' not in info else info['base_uri_parse'],enable_del=True if 'enable_del' not in info else info['enable_del'],merge_mode=3 if 'merge_mode' not in info else info['merge_mode'],headers=None if 'headers' not in info else info['headers'],work_dir=None if 'work_dir' not in info else info['work_dir'],method=None if 'method' not in info else info['method'],key=None if 'key' not in info else info['key'],proxy=None if 'proxy' not in info else info['proxy'])
